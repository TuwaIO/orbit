...
- **`@tuwaio/orbit-evm` (L2)**: Resolves EVM-specific communication primitives, public client caching, and ENS resolution powered by `viem` and `@wagmi/core`.
- **`@tuwaio/orbit-solana` (L2)**: Implements Solana RPC client caches, Wallet Standard connector discovery, and metadata lookups powered by `gill`.
